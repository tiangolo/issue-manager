...
README_ACTION_REF_PATTERN = re.compile(
    r"(?m)(tiangolo/issue-manager@)(\d+\.\d+\.\d+)"
)
...
def update_readme(content: str, current_version: str, version: str, readme_file: Path) -> str:
    if parse_version(version) <= parse_version(current_version):
        raise RuntimeError(
            f"New version {version} must be greater than current version {current_version}"
        )
    ...
    def replace_ref(match: re.Match[str]) -> str:
        ...
        if matched_version != current_version:
            raise RuntimeError(
                f"Found README issue-manager version {matched_version}, "
    ...
    if replacements == 0:
        raise RuntimeError(f"Could not find issue-manager action refs in {readme_file}")
    return new_content
